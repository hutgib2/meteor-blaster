...
class SpaceShooter():
	def __init__(self):
		...
		# import
		self.laser_surf = pygame.image.load(join('src', 'images', 'laser.png')).convert_alpha()
		...
		self.laser_sound = pygame.mixer.Sound(join('src', 'audio', 'damage.ogg'))
		self.laser_sound.set_volume(0.2)
		self.explosion_sound = pygame.mixer.Sound(join('src', 'audio', 'explosion.ogg'))
...
def collisions(self):
	...
	for laser in self.laser_sprites:
		...
		if collision:
			laser.kill()
			AnimatedExplosion(self.explosion_frames, laser.rect.midtop, self.self.all_sprites)
	...
	def display_score(self):
		...
		self.pygame.draw.rect(self.display_surface, '#F0F0F0', self.text_rect.inflate(20, 10).move(0, -6), 5, 10)
